...
void CLabel::OnPaint() 
{
	...
	else
	{
		...
		{
			CString szTotal = strText;
			int pos = szTotal.Find(_T("\r\n"));
			int x = rc.left, y = rc.top;
			while ((pos = szTotal.Find(_T("\r\n")))>=0)
			{
				CString strText1 = szTotal.Left(pos);
				szTotal = szTotal.Mid(pos+2);
				CSize cs = pDCMem->GetTextExtent(strText1);
				pDCMem->TextOut (x, y, strText1);
				y += cs.cy;
			}
			pDCMem->TextOut (x, y, szTotal);
		}
	}
...
//////////////////////////////////////////////////////////////////////////
CLabel& CLabel::SetText(LPCTSTR pstrText)
{
	if(IsWindow(this->GetSafeHwnd())) 
	{
		SetWindowText(pstrText?pstrText:_T(""));
		UpdateSurface();
	}
...
CLabel & CLabel::GetTextExtent(CSize &cs)
{
	CDC *pDC = GetDC();
	if (pDC)
	{
		CString szContent = _T("");
		GetWindowText(szContent);
		cs = pDC->GetTextExtent(szContent);
		ReleaseDC(pDC);
	}
...
//////////////////////////////////////////////////////////////////////////
CLabel& CLabel::SetFontName(LPCTSTR pstrFont, BYTE byCharSet /* Default = ANSI_CHARSET */)
{	
	if (pstrFont)
	{
		m_lf.lfCharSet = byCharSet;
		_tcscpy_s(m_lf.lfFaceName, pstrFont);
		ReconstructFont();
		UpdateSurface();
	}
...
// NT ALMOND				02072002    1.6     Origin
//////////////////////////////////////////////////////////////////////////
CLabel& CLabel::SetHyperLink(LPCTSTR psLink)
{
	m_sLink = psLink?psLink:_T("");

	return *this;
}

CLabel& CLabel::SetVerticalCenter(BOOL bSet)
{
	m_bVerticalCenter = bSet;
