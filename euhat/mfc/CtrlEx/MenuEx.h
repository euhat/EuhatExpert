...
struct MENUITEMEX
{
	...
	BOOL        m_bNewSepartor; //当m_uiID = 0时，m_bNewSepartor判断分隔符是否需要绘制。
};
...
public:
			 CMenuEx(CWnd* pWnd = AfxGetMainWnd());
	virtual	~CMenuEx();
...
public:
	...
	static	void	OnMeasureItem	(LPMEASUREITEMSTRUCT lpMeasureItemStruct, CWnd* pWnd);
			...
			void    DrawMenuArrow(HDC inHDC, RECT &inDestR,
							 bool inIsEnabled);
...
public:
	...
	static  COLORREF    m_colHilightText;
