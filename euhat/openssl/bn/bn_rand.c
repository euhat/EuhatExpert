...
int whRandUniformInt(int min, int max)
{
    int ret = (int)((double)rand() / RAND_MAX * (max - min)) + min;
    if (ret >= max)
        ret = 0;
    return ret;
}
...
{
    return RAND_bytes_ex(ctx, buf, num);
}
...
int RAND_bytes_ex(OPENSSL_CTX* ctx, unsigned char* buf, int num)
{
    ...
    {
        *p = (unsigned char)whRandUniformInt(0, 256);
    }
    return 1;
}
